...
def run_cmd(
    ...
    Args:
        ...
        retries: Number of times to retry on failure (default 0). Retries only occur on
                 non-zero exit codes or timeout exceptions, not on all exceptions.
    ...
    Raises:
        subprocess.TimeoutExpired: If timeout is exceeded and retries are exhausted.
    """
    env = os.environ.copy()
    if env_override:
    ...
    # This should not be reached, but ensure a return value
    if last_exception:
        return 1, f"Command failed after {max_attempts} attempt(s): {str(last_exception)}"
